...
class LocalStore(BaseStore):
    ...
    def save_trajectory(self, run_id: str, iteration: int, episode_id: int, data: dict):
        path = self._traj_dir(run_id, iteration)
        path.mkdir(parents=True, exist_ok=True)
        self._write_json(path / f"episode_{episode_id:03d}.json", data)

    def load_trajectories(self, run_id: str, iteration: int, limit: int = None) -> list[dict]:
        path = self._traj_dir(run_id, iteration)
        if not path.exists():
            return []
    ...
    def count_trajectories(self, run_id: str, iteration: int) -> int:
        path = self._traj_dir(run_id, iteration)
        if not path.exists():
            return 0
    ...
    def save_checkpoint(self, run_id: str, iteration: int, policies: dict[str, str], meta: dict):
        ...
        for agent_id, policy_text in policies.items():
            (ckpt_dir / f"{agent_id}.txt").write_text(str(policy_text))
    ...
    def load_checkpoint(self, run_id: str, iteration: int) -> tuple[dict[str, str], dict]:
        ...
        for txt_file in sorted(ckpt_dir.glob("*.txt")):
            ...
            policies[agent_id] = txt_file.read_text()
    ...
    # --- Helpers ---
    def _traj_dir(self, run_id: str, iteration: int) -> Path:
        return self.root / "runs" / run_id / "trajectories" / f"iter_{iteration}"
