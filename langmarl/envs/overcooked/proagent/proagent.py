...
try:
	...
	# Same surface as the handful of get_distribution(...).version calls below,
	# so those stay identical to the upstream ProAgent implementation.
	from importlib import metadata as _metadata

	class _Distribution:
		def __init__(self, name):
			self.version = _metadata.version(name)
...
from .llm_config import get_api_key, is_openrouter, get_llm_config
...
class ProAgent(object):
	...
	def load_openai_keys(self):
		# First try to load from llm_config.json
		try:
			api_key = get_api_key()
			if api_key:
				self.openai_api_keys = [api_key]
				return
		except Exception:
			pass

		# Fallback to openai_key.txt
	...
	def openai_api_key(self):
		if self.key_rotation and len(self.openai_api_keys) > 1:
...
class ProMediumLevelAgent(ProAgent):
	...
	def create_gptmodule(self, module_name, file_type='json', retrieval_method='recent_k', K=10):
		...
		model_name = self.layout

		prompt_file = os.path.join(PROMPT_DIR, model_name, f'{self.layout}_{self.agent_index}.{file_type}')
		policy_file = os.path.join(PROMPT_DIR, model_name, "policy.json")
		...
		with open(prompt_file, "r") as f:
			if file_type == 'json':
				...
				# Load dynamic policy from policy.json
				if os.path.exists(policy_file):
					with open(policy_file, "r") as pf:
						policy_data = json.load(pf)
						current_iter = policy_data.get('current_iteration', 0)
						iter_key = f"iteration_{current_iter}"

						if iter_key in policy_data['policies']:
							agent_key = f"agent_{self.agent_index}"
							if agent_key in policy_data['policies'][iter_key]:
								# Replace policy with the one from policy.json
								data['policy'] = policy_data['policies'][iter_key][agent_key]['policy']
								print(f"\n[ACTOR MODE] Loaded policy for Agent {self.agent_index} from iteration {current_iter}\n")

				# 将 JSON 字段组合成系统消息
				content_parts = []
				for key in ['instructions', 'policy', 'actions', 'facts', 'outputs', 'examples']:
					if key in data:
		...
		return Module(messages, self.model, retrieval_method, K)
	...
	def reset(self):
		...
		# For tracking transition information
		self.last_observation_to_gpt = None
		self.last_gpt_response = None
		self.last_parsed_ml_action = None

	def set_agent_index(self, agent_index):
		self.agent_index = agent_index
		self.planner = self.create_gptmodule("planner", retrieval_method=self.retrieval_method, K=self.K)

		print(self.planner.instruction_head_list[0]['content'])

	def get_transition_info(self, state, low_level_action):
		"""
		Get detailed transition information for logging.
		"""
		player = state.players[self.agent_index]
		return {
			"observation_to_gpt": self.last_observation_to_gpt,
			"gpt_query_response": self.last_gpt_response,
			"parsed_ml_action": self.last_parsed_ml_action,
			"low_level_action": Action.to_char(low_level_action) if low_level_action is not None else None,
			"low_level_state": {
				"position": list(player.position),
				"orientation": list(player.orientation),
				"held_object": player.held_object.name if player.held_object else "nothing"
			}
		}
	...
	def get_min_distance_to_locations(self, player_pos_and_or, target_locations):
		"""
		计算从玩家位置到一组目标位置中最近的距离。
		使用 motion planner 获取实际路径成本。
		"""
		...
		for target_pos in target_locations:
			# 获取到达该位置的所有可能 motion goals
			# 使用 motion_planner 的预计算字典获取目标位置的 motion goals
			possible_goals = self.mlam.motion_planner.motion_goals_for_pos.get(target_pos, [])
			for goal in possible_goals:
				if self.mlam.motion_planner.is_valid_motion_start_goal_pair(player_pos_and_or, goal):
					try:
						_, _, plan_cost = self.mlam.motion_planner.get_plan(player_pos_and_or, goal)
						if plan_cost < min_dist:
							min_dist = plan_cost
					except:
						pass
		return min_dist

	def generate_distance_comparison_prompt(self, state):
		"""
		生成两个 agent 到锅和 Dispenser 的距离信息。
		"""
		player0_pos_and_or = state.players_pos_and_or[0]
		player1_pos_and_or = state.players_pos_and_or[1]

		distance_prompt = "Distance info: "

		# Dish Dispenser 的距离
		dish_locations = self.mdp.get_dish_dispenser_locations()
		if dish_locations:
			player0_dish_dist = self.get_min_distance_to_locations(player0_pos_and_or, dish_locations)
			player1_dish_dist = self.get_min_distance_to_locations(player1_pos_and_or, dish_locations)
			distance_prompt += f"<Player 0> distance to dish dispenser: {player0_dish_dist}; <Player 1> distance to dish dispenser: {player1_dish_dist}. "

		# Onion Dispenser 的距离
		onion_locations = self.mdp.get_onion_dispenser_locations()
		if onion_locations:
			player0_onion_dist = self.get_min_distance_to_locations(player0_pos_and_or, onion_locations)
			player1_onion_dist = self.get_min_distance_to_locations(player1_pos_and_or, onion_locations)
			distance_prompt += f"<Player 0> distance to onion dispenser: {player0_onion_dist}; <Player 1> distance to onion dispenser: {player1_onion_dist}. "

		# Pot 的距离
		pot_locations = self.mdp.get_pot_locations()
		if pot_locations:
			player0_pot_dist = self.get_min_distance_to_locations(player0_pos_and_or, pot_locations)
			player1_pot_dist = self.get_min_distance_to_locations(player1_pos_and_or, pot_locations)
			distance_prompt += f"<Player 0> distance to pot: {player0_pot_dist}; <Player 1> distance to pot: {player1_pot_dist}. "
	...
	def generate_state_prompt(self, state):
		...
		if self.layout == 'forced_coordination' or self.layout == "coordination_ring" or self.layout == "counter_circuit": 
		...
		# 添加距离比较信息
		distance_prompt = self.generate_distance_comparison_prompt(state)

		return (self.layout_prompt + time_prompt + ego_state_prompt +
				teammate_state_prompt + kitchen_state_prompt + distance_prompt)
	...
	def action(self, state):
		...
		# ========== 打印：完整的底层状态 ==========
		print("\n" + "▼"*80)
		print(f"🎮 [LOW-LEVEL STATE] Timestep {state.timestep}")
		print("▼"*80)

		# 玩家位置和朝向
		print(f"\n📍 Player Positions and Orientations:")
		for i, pos_and_or in enumerate(state.players_pos_and_or):
			pos, orientation = pos_and_or
			print(f"   Player {i}: Position {pos}, Orientation {orientation}")

		# 玩家持有物
		print(f"\n🤲 Player Held Objects:")
		for i, player in enumerate(state.players):
			obj_name = player.held_object.name if player.held_object else "nothing"
			print(f"   Player {i}: {obj_name}")
		...
		for pos, obj in counter_objects.items():
			if obj is not None:
				print(f"   Counter at {pos}: {obj}")

		# 当前的中级动作
		print(f"\n🎯 Current Medium-level Action:")
		print(f"   Player {self.agent_index}: {self.current_ml_action}")
		print(f"   Steps executed: {self.current_ml_action_steps}")

		# 队友的中级动作（如果有）
		if state.ml_actions[1-self.agent_index] is not None:
			print(f"   Player {1-self.agent_index}: {state.ml_actions[1-self.agent_index]}")
		...
		# Validate action once, if invalid just wait (no retry to save LLM calls)
		...
		if "wait" in self.current_ml_action:
			...
			# ========== 打印：Wait 动作 ==========
			print("\n" + "⏳"*40)
			print(f"[WAIT ACTION] Player {self.agent_index}")
			print(f"   Waiting: {self.time_to_wait} timesteps remaining")
			print(f"   Random action: {Action.to_char(chosen_action)} ({chosen_action})")
			print("⏳"*40 + "\n")
			# ==========================================
			'''
			...
			# ========== 打印：运动目标 ==========
			print("\n" + "🎯"*40)
			print(f"[MOTION GOALS] For ml_action: {self.current_ml_action}")
			print(f"Player {self.agent_index} possible motion goals:")
			for i, mg in enumerate(possible_motion_goals):
				print(f"   Goal {i}: Position {mg[0]}, Orientation {mg[1]}")
			print("🎯"*40 + "\n")
			# ==========================================
			'''
		...
		# ========== 打印：最终的底层动作 ==========
		print("\n" + "⚡"*40)
		print(f"[FINAL ACTION] Player {self.agent_index}")
		print(f"   Medium-level action: {self.current_ml_action}")
		print(f"   Current motion goal: {current_motion_goal if 'current_motion_goal' in locals() else 'N/A'}")
		print(f"   Low-level action: {Action.to_char(chosen_action)} ({chosen_action})")
		print(f"   From position: {start_pos_and_or}")
		print("⚡"*40 + "\n")
		# ==========================================
		'''
	...
	def generate_ml_action(self, state):
		...
		print(f"   Parsed ml_action: '{ml_action}'")
		if "wait" in ml_action:
			print(f"   Wait time: {self.time_to_wait}")
		print()
		# ==========================================
		'''
		...
		# Store the information for transition tracking
		self.last_observation_to_gpt = state_prompt
		self.last_gpt_response = response
		self.last_parsed_ml_action = ml_action
	...
	def find_motion_goals(self, state):
		...
		if self.current_ml_action in ["pickup(onion)", "pickup_onion"]:
			...
			if pkg_resources.get_distribution("overcooked_ai").version == '1.1.0':
				...
				soups_ready_to_cook = pot_states_dict["onion"][soups_ready_to_cook_key] + pot_states_dict["tomato"][soups_ready_to_cook_key]
			...
			# 优先选择已经熟了的锅
			if pkg_resources.get_distribution("overcooked_ai").version == '1.1.0':
				ready_pots = pot_states_dict['ready']
			elif pkg_resources.get_distribution("overcooked_ai").version == '0.0.1':
				ready_pots = pot_states_dict['onion']['ready'] + pot_states_dict['tomato']['ready']
			
			print("ready_pots: ", ready_pots)
			if len(ready_pots) > 0:
				# 有熟了的锅，只考虑 ready 的锅
				motion_goals = am._get_ml_actions_for_positions(ready_pots)
			else:
				# 没有熟的锅，考虑正在煮的锅
