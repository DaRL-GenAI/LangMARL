...
class MediumLevelActionManager(object):
    ...
    def pickup_onion_from_dispenser_actions(self, state, player_positions, player_index):
        """只从 dispenser 拿 onion"""
        onion_dispenser_locations = self.mdp.get_onion_dispenser_locations()
        return self._get_ml_actions_for_positions(onion_dispenser_locations)

    def pickup_onion_from_counter_actions(self, state, counter_objects, player_positions, player_index):
        """只从 counter 拿 onion"""
        onion_counter_locations = counter_objects['onion']
        if len(onion_counter_locations) == 0:
            return []

        visitable_cur = get_visitable_positions(player_positions[player_index], self.mdp)
        visitable_oth = get_visitable_positions(player_positions[1 - player_index], self.mdp)

        # 优先选择只有当前玩家可达的 counter onion
        separate_states = []
        for loc in onion_counter_locations:
            possible_states = self._get_ml_actions_for_positions([loc])
            flag_cur, flag_oth = False, False
            for i in possible_states:
                if i[0] in visitable_cur:
        ...
        if len(separate_states) == 0:
            return self._get_ml_actions_for_positions(onion_counter_locations)
        else:
            return self._get_ml_actions_for_positions(separate_states)
