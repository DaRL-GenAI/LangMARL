#!/usr/bin/env python3
"""Train 2-agent math task using gpt-3.5-turbo (actor) + gpt-4o-mini (critic)."""
...
config = langmarl.LanguageTaskConfig(
    exp_name="math_2agent",
    paradigm="central_credit",
    task_type="math",
    benchmark_path=os.path.join(os.path.dirname(__file__), "..", "env", "lang_benchmark", "MATH"),
    data_limit=100,
    num_agents=2,
    num_iterations=5,
    ...
    checkpoint_dir=os.path.join(os.path.dirname(__file__), "..", "experiments", "ckpt_math"),
...
if __name__ == "__main__":
    print("=" * 60)
    print("LangMARL Math Task Training")
